...
6. Данные хранятся в оперативной памяти сервера с записью в хронологическом порядке модифицирующих операций в лог. После перезапуска сервера база восстанавливается из лога, после операции восстановления файл с логом минимизируется (в полученном файле остаются только конечные операции для каждого ключа).
